...
static void RenderWelcome(App *app) {
  ASSERT(!app->document);
...
static void DrawMenuBar(App *app, Arena scratch) {
  ...
  if (ImGui::BeginMainMenuBar()) {
    ...
    if (app->document) {
      ...
      Vec2 size = ImGui::CalcTextSize((char *)path.data,
      ...
      draw_list->AddText(ImGui::GetFont(), ImGui::GetFontSize(),
                         {from, ImGui::GetItemRectMin().y}, IM_COL32_BLACK,
                         (char *)path.data, (char *)(path.data + path.size),
                         0.0f, &clip_rect);
    }
...
static bool AppCanLoadFile(App *app) { return true; }
