...
TEST_F(TrackRendererTest, CoalesceSameColor) {
  Track t = {};
  // Create 3 tiny events close to each other with same color (by using same name)
  TraceEvent e1 = {STR("e"), STR("cat"), STR("B"), STR(""), 100, 10, 0, 0, nullptr, 0};
  TraceEvent e2 = {STR("e"), STR("cat"), STR("B"), STR(""), 112, 10, 0, 0, nullptr, 0};
  TraceEvent e3 = {STR("e"), STR("cat"), STR("B"), STR(""), 125, 10, 0, 0, nullptr, 0};
  ...
  track_compute_render_blocks(&t, &td, 0, 10000, 1000.0f, 0.0f, -1, &state, &blocks, allocator);
...
TEST_F(TrackRendererTest, DoNotCoalesceDifferentColors) {
  Track t = {};
  TraceEvent e1 = {STR("e1"), STR("cat"), STR("B"), STR(""), 100, 10, 0, 0, nullptr, 0};
  TraceEvent e2 = {STR("e2"), STR("cat"), STR("B"), STR(""), 112, 10, 0, 0, nullptr, 0};
  ...
  // BUT the second one might be skipped if it falls in the same pixel range (LOD)
  // 112*0.1 = 11.2, 100*0.1+1 = 11.0. 11.2 > 11.0, so it's NOT skipped by LOD.
...
TEST_F(TrackRendererTest, SelectedEventNeverSkipped) {
  Track t = {};
  TraceEvent e1 = {STR("e1"), STR("cat"), STR("B"), STR(""), 100, 10, 0, 0, nullptr, 0};
  TraceEvent e2 = {STR("e2"), STR("cat"), STR("B"), STR(""), 101, 10, 0, 0, nullptr, 0}; // Overlaps e1 almost completely
...
TEST_F(TrackRendererTest, ExtremeZoomOut) {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    TraceEvent e = {str_from_cstr(name), STR("cat"), STR("B"), STR(""), (int64_t)i * 2, 1, 0, 0, nullptr, 0};
  ...
  track_compute_render_blocks(&t, &td, 0, 1000000, 1000.0f, 0.0f, -1, &state, &blocks, allocator);
  ...
  // x2 will slightly increase each time, potentially bypassing the current check.
