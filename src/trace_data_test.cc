...
TEST(trace_data_test, basic) {
  ...
  trace_arg_t args[2];
  args[0] = {SV("key1"), SV("val1"), 0.0};
  args[1] = {SV("key2"), SV("val2"), 0.0};
  ev.args = args;
  ev.args_count = 2;
...
TEST(trace_data_test, de_duplication) {
  ...
  string_ref_t ref2 = trace_data_push_string(td, SV("bar"), a);
  string_ref_t ref3 = trace_data_push_string(td, SV("foo"), a);
...
TEST(trace_data_test, begin_end_events_nested_and_thread_isolated) {
  ...
  // 1. Thread 1 B1
  trace_event_t b1 = {
      .name = SV("parent"), .ph = SV("B"), .ts = 100, .pid = 1, .tid = 1};
  trace_data_add_event(td, &b1, &matcher, a);

  // 2. Thread 2 B2 (Different thread)
  trace_event_t b2 = {
      .name = SV("other"), .ph = SV("B"), .ts = 110, .pid = 1, .tid = 2};
  trace_data_add_event(td, &b2, &matcher, a);

  // 3. Thread 1 B3 (Nested on thread 1)
  trace_event_t b3 = {
      .name = SV("child"), .ph = SV("B"), .ts = 120, .pid = 1, .tid = 1};
  ...
  trace_event_t e3 = {.ph = SV("E"), .ts = 130, .pid = 1, .tid = 1};
  ...
  trace_event_t e2 = {.ph = SV("E"), .ts = 140, .pid = 1, .tid = 2};
  ...
  trace_event_t e1 = {.ph = SV("E"), .ts = 150, .pid = 1, .tid = 1};
  trace_data_add_event(td, &e1, &matcher, a);
...
TEST(trace_data_test, begin_end_events_args_merging) {
  ...
  b_args[0] = {SV("arg1"), SV("val1"), 0.0};
  b_args[1] = {SV("arg2"), SV(""), 42.0};
  b.args = b_args;
  b.args_count = 2;
  ...
  e_args[0] = {SV("arg2"), SV(""), 99.0};     // Overrides arg2
  e_args[1] = {SV("arg3"), SV("val3"), 0.0};  // New arg
  e.args = e_args;
  e.args_count = 2;
