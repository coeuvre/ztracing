...
static JsonParser InitJsonParser(GetJsonInputFunc get_json_input, void *data) {
  JsonParser parser = {};
...
static void ReturnInput(JsonParser *parser, u8 val) {
  ASSERT(parser->tmp == 0);
  parser->tmp = val;
}

static u8 TakeInput(JsonParser *parser) {
  u8 val = parser->tmp;
  if (val) {
    parser->tmp = 0;
  } else {
...
static Buffer TakeInput(Arena *arena, JsonParser *parser, isize count) {
...
static bool IsJsonWhitespace(u8 val) {
...
static inline void Append(Arena *arena, Buffer *buffer, isize *cursor, u8 val) {
...
static inline bool ParseDigits(JsonParser *parser, Arena *arena, Buffer *buffer,
                               isize *cursor) {
...
static JsonToken GetJsonToken(Arena *arena, JsonParser *parser) {
  JsonToken token = {};
  ...
  switch (val) {
    ...
    case 't': {
      ...
      if (Equal(expected_suffix, suffix)) {
        ...
        token.value = PushFormat(arena, "expecting 'true', but got 't%.*s'",
    ...
    case 'f': {
      ...
      if (Equal(expected_suffix, suffix)) {
        ...
        token.type = JsonToken_Error;
        token.value = PushFormat(arena, "expecting 'false', but got 'f%.*s'",
                                 (int)suffix.size, suffix.data);
      }
    } break;
    ...
    case 'n': {
      ...
      if (Equal(expected_suffix, suffix)) {
        ...
        token.type = JsonToken_Error;
        token.value = PushFormat(arena, "expecting 'null', but got 'n%.*s'",
                                 (int)suffix.size, suffix.data);
      }
    } break;
        ...
        if (val == '.') {
          ...
          if (!ParseDigits(parser, arena, &buffer, &cursor)) {
            ...
            token.value = PushFormat(
                ...
                "got '%c'",
                (int)buffer.size, buffer.data, val);
            done = true;
          }
        } else {
          ReturnInput(parser, val);
...
static void ParseJsonValue(Arena *arena, Arena scratch, JsonParser *parser,
                           JsonValue *result);

static void ParseJsonObject(Arena *arena, Arena scratch, JsonParser *parser,
                            JsonValue *result) {
    ...
    switch (token.type) {
      case JsonToken_StringLiteral: {
        ...
        if (token.type == JsonToken_Colon) {
          ...
          result->type = JsonValue_Error;
          result->value = PushFormat(arena, "expecting ':', but got %.*s",
                                     (int)token.value.size, token.value.data);
          done = true;
        }
      } break;
      ...
      default: {
        result->type = JsonValue_Error;
        result->value = PushFormat(arena, "Unexpected token '%.*s'",
                                   (int)token.value.size, token.value.data);
        done = true;
      };
    }
...
static void ParseJsonArray(Arena *arena, Arena scratch, JsonParser *parser,
                           JsonValue *result) {
  JsonValue *last_child = 0;
  bool done = false;
  while (!done) {
    ...
    if (child->type != JsonValue_Error) {
      ...
      switch (token.type) {
        ...
        default: {
          result->type = JsonValue_Error;
          result->value =
              PushFormat(arena, "expecting token ',' or '], but got %.*s",
                         (int)token.value.size, token.value.data);
          done = true;
        } break;
      }
...
static JsonValue *GetJsonValue(Arena *arena, Arena scratch,
                               JsonParser *parser) {
...
static f64 ConvertSign(Buffer buffer, isize *cursor) {
  f64 sign = 1.0;
...
static f64 ConvertNumber(Buffer buffer, isize *out_cursor) {
  f64 result = 0.0;
