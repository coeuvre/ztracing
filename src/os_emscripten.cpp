...
static OsLoadingFile *OsOpenFile(char *path) { return 0; }
...
static void NotifyAppInitDone() { AppSetupResolve(); }
...
extern "C" void *AppAllocateMemory(isize size) {
...
extern "C" void AppSetWindowSize(int width, int height) {
  SDL_SetWindowSize(kMainLoop.window, width, height);
}

EMSCRIPTEN_KEEPALIVE
extern "C" bool AppOnLoadBegin(char *path, isize total) {
  bool result = !kMainLoop.loading_file && AppCanLoadFile(kMainLoop.app);
  ...
  if (result) {
    ...
    kMainLoop.loading_file = file;
    AppLoadFile(kMainLoop.app, file);
  }
...
EMSCRIPTEN_KEEPALIVE
extern "C" bool AppOnLoadChunk(u8 *buf, isize len) {
  OsLoadingFile *file = kMainLoop.loading_file;
